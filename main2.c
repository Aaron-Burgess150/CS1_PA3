...
/*
    PROGRAM 2 (returns the first lexicographical permutation found)
    name are in alphabetical orde rso just the first permutation found
*/
...
//function prototypes
void printPerm(Guest *perm, int *used, int k, int numP, Guest *attendees, int conflicts[MAXP][MAXP]);
...
int main(void) {
    ...
    printPerm(perm, used, 0, numP, attendees, conflicts); //recursive function to get a valid permutation and print it
}

void printPerm(Guest *perm, int *used, int k, int numP, Guest *attendees, int conflicts[MAXP][MAXP]) {
    //once you get a permutation, check it's validity. If valid, print it and exit main2
    if (k==numP) { //when you get a perm
        if (checkConflict(perm, attendees, conflicts, numP)) { //if there is no conflict, it's valid
            for (int i = 0; i < numP; i++) { //print valid permutation
                printf("%s\n", perm[i].name);
            }
            exit(0);
        }
    ...
    for (i = 0; i<numP; i++) {
        if (!used[i]) { //if it's not used
            ...
            if (checkSnacks(perm, k, numP)){ //if snack is fine, run it recursively to finish the permutation
                printPerm(perm, used, k+1, numP, attendees, conflicts);
            }
